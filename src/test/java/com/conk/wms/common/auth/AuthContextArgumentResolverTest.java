...
class AuthContextArgumentResolverTest {
    ...
    void supportsParameter_whenAuthContextType_returnsTrue() throws NoSuchMethodException {
        AuthContextArgumentResolver resolver = new AuthContextArgumentResolver(objectProvider(new AuthContextResolver(true)));
    ...
    void resolveArgument_returnsResolvedAuthContext() throws NoSuchMethodException {
        ...
        request.addHeader(AuthHeaders.WORKER_CODE, "WORKER-001");
        ...
        assertThat(resolved.getWorkerCode()).isEqualTo("WORKER-001");
        assertThat(resolved.getRole()).isEqualTo(AuthRole.WH_WORKER);
        assertThat(resolved.getTenantId()).isEqualTo("tenant-001");
    }

    private ObjectProvider<AuthContextResolver> objectProvider(AuthContextResolver resolver) {
        @SuppressWarnings("unchecked")
        ObjectProvider<AuthContextResolver> provider = mock(ObjectProvider.class);
        ...
        when(provider.getIfAvailable(org.mockito.ArgumentMatchers.any())).thenAnswer(invocation -> {
            @SuppressWarnings("unchecked")
            java.util.function.Supplier<AuthContextResolver> supplier = invocation.getArgument(0);
            return supplier.get();
        });
        return provider;
    }
